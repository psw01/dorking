...
const SearchHistory: React.FC<SearchHistoryProps> = ({ onLoadSearch }) => {
  ...
  const handleBookmarkToggle = (id: string, isBookmarked: boolean) => {
    ...
    // Update local state
    setHistoryItems(prev => 
      prev.map(item => item.id === id ? { ...item, bookmarked: !isBookmarked } : item)
    );
    ...
    toast({
      ...
      description: isBookmarked 
        ? "Search removed from bookmarks" 
        : "Search added to bookmarks",
    });
  };
  ...
  const filteredItems = showBookmarkedOnly 
    ? historyItems.filter(item => item.bookmarked) 
    : historyItems;
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center space-x-2">
          <Button 
            ...
            className={showBookmarkedOnly ? "bg-cyber-teal hover:bg-cyber-teal/90" : ""}
          >
            <Star className={`h-4 w-4 mr-2 ${showBookmarkedOnly ? "" : "text-cyber-teal"}`} />
            {showBookmarkedOnly ? "All Searches" : "Bookmarks Only"}
          </Button>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {filteredItems.length === 0 ? (
          <Card className="cyber-card p-8 text-center">
            <Star className="mx-auto h-12 w-12 text-muted-foreground mb-2 opacity-70" />
            <h3 className="text-lg font-mono mb-2">No Bookmarked Searches</h3>
            <p className="text-muted-foreground">
              Bookmarked searches will appear here when you star items.
            </p>
          </Card>
        ) : (
          filteredItems.map((item) => (
            ...
            } ${item.bookmarked ? 'ring-1 ring-cyber-blue/50' : ''}`}>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-4 pb-2">
                  ...
                  <div className="flex items-center space-x-1 mt-2 sm:mt-0">
                    <Button
                      ...
                      className={`h-8 w-8 ${item.bookmarked ? 'text-cyber-blue' : 'text-muted-foreground'}`}
                    >
                      <Star className="h-4 w-4" fill={item.bookmarked ? "currentColor" : "none"} />
                    </Button>
                ...
                {item.notes && (
                  <div className="px-4 pb-3">
                    ...
                    <div className="text-xs">
                      ...
                      <p className="font-mono mt-1 whitespace-pre-wrap line-clamp-4 bg-muted/30 p-2 rounded">
                        {item.notes}
                      </p>
                    </div>
                  </div>
